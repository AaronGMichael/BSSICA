...
def corrplot():
    ...
    SOURCE_FILE, sampling_rate = sf.read("Resources/StrTruLapBassplucks.wav")
    TARGET_FILE, sampling_rate2 = sf.read("Resources/Outputs/split2sig2.wav")
    ...
    fig = plt.figure()
    plt.subplot(4, 1, 1)
    plt.suptitle('Plucks(guitar) Correlation for String+Trumpet+Bass+Plucks Mix')
    plt.scatter(x, y)
    plt.plot(np.unique(x), np.poly1d(np.polyfit(x, y, 1))(np.unique(x)), color='red')
    plt.xlabel('x axis')
    plt.ylabel('y axis')
